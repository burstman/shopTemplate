...
ifeq ($(OS),Windows_NT)
	...
	--pattern "**/*.css" \
	--delay "100ms" \
	-- curl -X POST -s -o /dev/null http://localhost:7331/_templ/reload
endif
...
watch-assets:
	...
	@npx tailwindcss -i app/assets/app.css -o ./public/assets/styles.css --watch=always
...
dev:
	@make -j6 templ server watch-assets watch-esbuild sync_assets
